...
│   ├── announce.go                      — AnnounceUseCase (подписки, рассылка с троттлингом)
...
│   ├── announce.go, announce_session.go — /subscribe и /announce
...
| `/create_tables` | Обновить таблицы всех бассейнов с идущим рейдом (то же, что `all`) |
...
| `/cancel` | Отменить активный диалог (`/edit_tables` или `/announce`) |
...
Определение недели различает три состояния рейда (`domain.RaidStatus`): рейд ещё не начался (идёт регистрация), идёт сейчас, закончился. Таблицу защит можно строить только для идущего или закончившегося рейда — в период регистрации команд ещё нет.
...
- `/announce` доступен любому одобренному админу: выбор аудитории (бассейн или все подписчики) → текст → предпросмотр с числом получателей → подтверждение. После рассылки приходит отчёт «успешно / не доставлено» с ID проблемных пользователей;
- отправка идёт с паузой 50 мс между сообщениями (~20 msg/s), чтобы не упереться в лимит Telegram (~30 msg/s);
- пока рассылка включена, подписчику приходят и cron-анонсы (FAQ, экзамен, финал, хакатон) по выбранным бассейнам. Рассылка в группы из `CHAT_IDS` работает как раньше;
- текст анонса экранируется как HTML: анонсы — простой текст, зато сообщение с `<` или `&` гарантированно доставляется.
...
| `usecase` | Расчёт таблицы защит (авто от 17:30 и ручное окно), DetectCurrentWeek / ForEvent во всех ветвях включая период регистрации, BuildDefenseReminder, подписки и рассылка анонсов | 34 |
| `config` | Загрузка env, обязательные поля, defaults, парсинг ChatIDs, схема URL, дубли SHEET_* | 17 |
| `delivery/telegram` | Авторизация, parsePiscineFromCallback, nextMonday, разбор аргумента /create_tables, отказ при общей таблице, экраны /subscribe и отчёт /announce | 19 |
| `infra/oneedu` | parseJWTExpiry (валидный, малформед, без exp), extractToken | 10 |
